...
const EXAMPLE_CITY_CODE = 'Ritawas.here';
...
const EXAMPLE_CITY = {
  ...
  districtNames: {
    shrine:      'By the Pearl River',
    garden:      'Denver Botanical Gardens',
    cornerstore: 'Zetcher 2',
    tower:       'Geese Park',
    plaza:       'Sam Fox Studio',
  },
  answers: {},
};

// city names the example city has used in the past — used to purge stale
// entries from browsers that saved an example city before this code existed
const LEGACY_EXAMPLE_CODES = ['NADIA-X7', 'Ritawas.here', 'RITAWAS.HERE'];

function seedExampleCity() {
  let log = JSON.parse(localStorage.getItem('visitor-log') || '[]');
  // drop any previously seeded example entry (flagged, or a known legacy code)
  // so renaming/restyling the example city doesn't leave duplicates behind
  log = log.filter(entry => !entry.isExample && !LEGACY_EXAMPLE_CODES.includes(entry.code));
  log.push({ code: EXAMPLE_CITY.code, cityName: EXAMPLE_CITY.cityName, addedAt: Date.now(), data: EXAMPLE_CITY, isExample: true });
  ...
  log.forEach((entry, i) => {
    ...
    row.innerHTML = `
      <div class="visitor-city-info">
        ...
        <div class="visitor-city-meta mono">${entry.code}</div>
      </div>
...
function addVisitorCity() {
  ...
  let cityData = null;
  if (code === EXAMPLE_CITY_CODE.toUpperCase()) {
...
function initConstellationBtn() {
  ...
  let btn = document.getElementById('constellation-btn');
  if (!btn) return;

  // clone to strip any listeners left over from a previous bind
  // (e.g. the visit-mode click handler attached by initVisitConstellationBtn)
  const fresh = btn.cloneNode(true);
  btn.parentNode.replaceChild(fresh, btn);
  btn = fresh;
  ...
  var topoSketch = new p5(function(sk) {
    ...
    sk.mouseMoved = function() {
      ...
      if (found !== hovRing) {
        ...
        if (found !== null) {
          var r = rings[found];
          htip.innerHTML = '<div style="font-size:15px;color:' + r.color + ';letter-spacing:0.04em;margin-bottom:5px;">' + r.log.name + '</div>'
            + '<div style="font-size:11px;color:var(--blue);opacity:0.45;">' + (r.log.date || '') + ' · ' + r.dist + '</div>'
            + '<div style="font-size:11px;color:var(--blue);opacity:0.65;margin-top:5px;">Click to visit entry</div>';
    ...
    sk.mousePressed = function() {
      if (hovRing === null) return;
      var r = rings[hovRing];
      localStorage.setItem('currentDistrict', r.dist);
      var url = 'districts/' + r.dist + '-customize.html';
      var label = r.dist.charAt(0).toUpperCase() + r.dist.slice(1);
      if (typeof window.navigateWithLoader === 'function') {
        window.navigateWithLoader(url);
        var labelEl = document.getElementById('page-loader-label');
        if (labelEl) labelEl.textContent = 'Entering ' + label;
      } else {
...
function initNewConstellationSketch() {
  ...
  if (allKeywords.length === 0) {
    container.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;font-family:var(--font-whois);font-size:0.78rem;color:var(--blue);opacity:0.35;letter-spacing:0.06em;">write a bit more in a district to see your constellation</div>';
    return;
  }
...
function renderReportView(container) {
  ...
  container.innerHTML = [
    ...
    '<div style="position:absolute;inset:0;margin:0 4rem;background-color:var(--color-bg);overflow-y:auto;">',
