...
function extractKeywords(sessions, topN = 8) {
  const freq = {};
...
function collectCityData(userName) {
  ...
  DISTRICTS.forEach(key => {
    ...
    const validSessions = sessions
      .filter(s => !s.isTrainThought && s.answers && s.answers[0])
...
function buildCityReceiptBytes(userName) {
  ...
  wrap(data.cityName.toUpperCase(), 20).forEach(l => parts.push(cmd.textLine(l)));
  parts.push(cmd.doubleWidth(false));
  parts.push(cmd.bold(false));

  if (data.userName) parts.push(cmd.textLine(toSentenceCase(data.userName)));
  parts.push(cmd.textLine('Your emotional hometown'));
  ...
  data.districts.forEach(d => {
    ...
    parts.push(cmd.bold(false));
    wrap('. ' + d.realPlace, PAPER_WIDTH).forEach((line, i) => {
      parts.push(cmd.textLine(i === 0 ? line : '  ' + line.trim()));
    });
    parts.push(cmd.textLine('  ' + d.dateLogged));
  ...
  parts.push(cmd.alignCenter());
  parts.push(cmd.textLine('-'.repeat(PAPER_WIDTH)));
  parts.push(cmd.textLine('Your city is always being built.'));
...
function buildReceiptBytes() {
  ...
  return buildCityReceiptBytes(userName);
}
...
async function sendToDevice(bytes) {
  ...
  for (let offset = 0; offset < bytes.length; offset += CHUNK) {
    await usbDevice.transferOut(usbEndpoint, bytes.slice(offset, offset + CHUNK));
  }
}
...
async function printReceipt() {
  ...
  setStatus('Printing...', '');
  try {
    await sendToDevice(buildReceiptBytes());
